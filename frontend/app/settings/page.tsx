...
import { User, Download, LogOut, Shield, Bell } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  const [toggles, setToggles] = useState({ reminders: true, digest: false });
        ...
        <div className="mt-10 space-y-8 max-w-xl">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0 }}
            ...
            <div className="divide-y divide-pink-light/20">
              <div className="flex items-center justify-between px-5 py-4">
                <span className="text-[14px] text-dark font-sans font-medium">Display name</span>
            ...
            <div className="divide-y divide-pink-light/20">
              {[
                ...
                <div key={item.id} className="flex items-center justify-between px-5 py-4">
                  ...
                  <button
                    onClick={() => setToggles((t) => ({ ...t, [item.id]: !t[item.id as keyof typeof t] }))}
                    className={`w-11 h-6 rounded-full relative transition-colors duration-300 ${toggles[item.id as keyof typeof toggles] ? 'bg-coral' : 'bg-muted/30'}`}
                  >
                    <motion.div
                      className="w-4 h-4 bg-white rounded-full absolute top-1 shadow-sm"
                      animate={{ left: toggles[item.id as keyof typeof toggles] ? '24px' : '4px' }}
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.16 }}
            ...
            <div className="divide-y divide-pink-light/20">
              ...
              <div className="px-5 py-4">
                <p className="text-[14px] text-dark font-sans font-medium">Change password</p>
                <p className="text-[12px] text-muted font-sans mt-1">Coming soon — this feature is being built.</p>
              </div>
              <div className="px-5 py-4">
                <button
                  onClick={() => {
